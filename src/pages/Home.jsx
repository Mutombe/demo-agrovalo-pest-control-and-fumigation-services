    ...
    <section ref={containerRef} className="relative h-screen min-h-[700px] overflow-hidden bg-white">
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        <AnimatePresence mode="sync">
          <motion.img key={currentSlide} src={heroImages[currentSlide]} alt={hero.backgroundImages[currentSlide]?.alt}
            ...
            transition={{ duration: 1.8, ease: 'easeInOut' }} loading="eager" />
        </AnimatePresence>
        <div className="absolute inset-0 bg-gradient-to-b from-white/30 via-white/10 to-emerald-950/80 z-[1]" />
        <div className="absolute inset-0 bg-gradient-to-r from-emerald-950/60 via-transparent to-emerald-950/20 z-[1]" />
      </motion.div>

      <NoiseTexture opacity={0.03} />

      {/* Eco leaf badge */}
      <div className="absolute top-28 right-6 sm:right-10 z-30 bg-white/95 backdrop-blur-sm shadow-lg p-4 flex items-center gap-3">
        <div className="w-10 h-10 bg-emerald-500 flex items-center justify-center">
          <Leaf size={20} weight="fill" className="text-white" />
        </div>
        <div>
          <div className="text-emerald-700 text-[10px] font-bold uppercase tracking-[0.15em]">Eco Certified</div>
          <div className="text-gray-400 text-[9px] uppercase tracking-wider">Safe & Green</div>
        </div>
      </div>

      <motion.div className="relative z-20 flex flex-col justify-end h-full max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 pb-20 pt-36" style={{ y: textY, opacity }}>
        <motion.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 0.8, delay: 0.3 }}
          className="w-16 h-[3px] bg-emerald-500 mb-6 origin-left" />
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }}
          className="text-emerald-300 text-xs sm:text-sm font-semibold uppercase tracking-[0.3em] mb-6" style={{ fontFamily: 'var(--font-sans)' }}>
          {hero.badge}
        </motion.p>
        <div className="overflow-hidden">
          {['PROTECT YOUR', 'SPACE', 'PRESERVE YOUR HEALTH.'].map((line, i) => (
            <motion.div key={line} initial={{ y: '110%' }} animate={{ y: 0 }} transition={{ duration: 1, delay: 0.5 + i * 0.15, ease: [0.16, 1, 0.3, 1] }}>
              <h1 className={`font-heading leading-[0.92] tracking-tight ${line === 'SPACE' ? 'text-emerald-400 italic' : 'text-white'}`}
                style={{ fontSize: 'clamp(2.2rem, 7vw, 4.5rem)', fontWeight: line === 'SPACE' ? 700 : 300 }}>{line}</h1>
            </motion.div>
          ))}
        </div>
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 1.2 }} className="flex items-center gap-3 mt-8">
          <div className="w-8 h-[1px] bg-emerald-400/40" />
          <p className="text-white/40 text-xs sm:text-sm uppercase tracking-[0.2em]" style={{ fontFamily: 'var(--font-sans)' }}>
            {business.projectsCompleted} Properties &middot; {business.yearsExperience} Experience &middot; Est. {business.established}
          </p>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 1.4 }} className="flex flex-wrap gap-4 mt-10">
          <Link to="/contact" className="group inline-flex items-center gap-3 bg-emerald-500 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold transition-all duration-500 hover:bg-emerald-600 hover:shadow-xl hover:shadow-emerald-500/20" style={{ fontFamily: 'var(--font-sans)' }}>
            {hero.ctaPrimary} <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" />
          </Link>
          <Link to="/services" className="group inline-flex items-center gap-3 border border-white/20 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold transition-all duration-500 hover:border-emerald-400/50 hover:text-emerald-300 hover:bg-white/5" style={{ fontFamily: 'var(--font-sans)' }}>
            {hero.ctaSecondary}
          </Link>
        </motion.div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.5 }}
        ...
        <motion.div animate={{ y: [0, 8, 0] }} transition={{ repeat: Infinity, duration: 2, ease: 'easeInOut' }}>
          <ArrowDown size={14} className="text-emerald-400/50" />
        </motion.div>
      </motion.div>
    </section>
  );
        ...
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9 }}>
            ...
            <h2 className="font-heading text-gray-900 leading-[0.95] italic mb-8" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>
              The Agrovalo<br /><span className="text-emerald-500">Story</span>
            </h2>
            {about.story.map((p, i) => (
        ...
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-20 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}}
            ...
            <h2 className="font-heading text-gray-900 leading-[0.95] italic mb-12" style={{ fontSize: 'clamp(2rem, 4.5vw, 3rem)' }}>
              Why Choose <span className="text-emerald-500">Agrovalo</span>
            </h2>
...
function ServiceAreaSection() {
  const ref = useRef(null);
  const inView = useInView(ref, { once: true, margin: '-80px' });
  const areas = ['Harare CBD', 'Borrowdale', 'Mt Pleasant', 'Avondale', 'Highlands', 'Greendale', 'Mabelreign', 'Eastlea'];
  return (
    <section ref={ref} className="bg-white py-20 sm:py-28">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="text-center mb-12">
          <div className="w-12 h-[2px] bg-emerald-500 mx-auto mb-6" />
          <h2 className="font-heading text-gray-900 leading-[0.95] italic" style={{ fontSize: 'clamp(1.8rem, 4vw, 3rem)' }}>
            Areas We <span className="text-emerald-500">Serve</span>
          </h2>
        </motion.div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {areas.map((area, i) => (
            <motion.div key={area} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
              transition={{ delay: i * 0.06 }} className="flex items-center gap-3 p-4 border border-emerald-200 bg-emerald-50 hover:border-emerald-500/50 transition-colors">
              <MapPin size={16} weight="fill" className="text-emerald-500 shrink-0" />
              <span className="text-gray-900 text-sm font-medium" style={{ fontFamily: 'var(--font-sans)' }}>{area}</span>
            </motion.div>
          ))}
        </div>
      </div>
...
function CTASection() {
  ...
  return (
    <section ref={ref} className="relative py-28 sm:py-36 lg:py-48 overflow-hidden">
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        <img src={siteData.hero.backgroundImages[0]?.url} alt="Agrovalo pest control and fumigation services" className="w-full h-[130%] object-cover object-center" loading="lazy" />
